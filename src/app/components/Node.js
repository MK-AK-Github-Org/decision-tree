                            ...
                            <div className='ml-10' key={i}>
                                {
                                    c.subcriteria.length > 0
                                        ? <Node criteria={c.subcriteria} operator={c.subcriterion_logical_operator} description={c.criterion_description} />
                                        : <Description description={c.criterion_description} />
                                }
                                {i < criteria.length - 1 ? <p className='text-sm font-bold'>{operator.toUpperCase()}</p> : null}
                            </div>
                        ))}
                    </motion.div>
                )}
...
function Description({ description }) {
    ...
    return (
        <div className={`relative text-lg font-medium w-fit max-w-5xl py-4 px-2 border my-2
            ${approved === 'neutral'
